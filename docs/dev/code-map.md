...
| the strip's cooldown sweep: League's radial clock cut to a square, shared by the weapon well and the four ability wells, rasterised a pixel at a time and coalesced into one fillRect a run (the two meal buttons keep the top-down wipe, and `AB_COVER` with it) | `drawSweepCover`/`CD_SWEEP`/`CD_EDGE` | `UI` › `hud strip` |
