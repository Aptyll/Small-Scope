...
| the ability cooldown sweep: League's radial clock cut to a square, rasterised a pixel at a time and coalesced into one fillRect a run (the long clocks only - the tool well and the meal buttons keep their top-down wipe) | `drawSweepCover`/`AB_SWEEP`/`CD_EDGE` | `UI` › `hud strip` |
