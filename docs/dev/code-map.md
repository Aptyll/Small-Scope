...
| the plates' own art: the merchant's gold sack and the three tails | `SPRITES.goldSack` (six frames: the 32x32 grids `sackA`..`sackF`/`SACKPAL` in the `gold sack` section of sprites.js, baked at 16 through `bakeHalf`), `NOTE_TAILS` (up, down, crate), `NOTE_CRATE_PAL` | `market notices` (the tails stamped with `stampGrid`: `the art`, screens.js) |
