...
| the roll station: random tracks, the die, the difficulty stones | `PK_DIFFS`/`PK_DIFF`, `PK_DIE`/`PK_STONES`, `pkTiles`, `pkGenPath`, `pkRoll`, `pkPlaceStones`, `pkPadNear`, `pkPadPress` (the press: keydown, input.js; the `E ROLL` cap: `drawPkHint`, ui.js) | `practice arena` (pixels: `drawPkDie`/`drawPkStone` + `PK_PIP_*`, draw-world.js) |
