...
| a picked bush's regrow clock, and the frame the bush wears at each stage of it | `BUSH_REGROW`/`BUSH_BUD_T`/`BUSH_RIPEN_T` | `world` (the pick: `hitObject`'s bush branch, actions.js; the tick: the object-timers loop, sim.js; the frame: the bush branch of `render()`, render.js; the grids: `bush`/`bushEmpty`/`bushBud`/`bushRipen`, sprites.js) |
