...
| the roll station: random tracks, the die, the difficulty stones, the carving-front sweep | `PK_DIFFS`/`PK_DIFF`, `PK_CX`/`PK_CY`/`PK_RX`/`PK_RY`/`PK_APRON`, `PK_DIE`/`PK_STONES`, `pkTiles`, `pkGenPath`, `pkPlanCarve`/`pkIceTile`, `pkRoll`, `pkAngKey`/`pkAnimStep`/`pkAnim`/`PK_ANIM_T`, `pkPlaceStones`, `pkPadNear`, `pkPadPress` (the press: keydown, input.js; the `E ROLL` cap: `drawPkHint`, ui.js) | `practice arena` (pixels: `drawPkDie`/`drawPkStone` + `PK_PIP_*`, draw-world.js) |
