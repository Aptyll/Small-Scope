...
| Looking for | Start at | Banner |
| --- | --- | --- |
| an animal's level: dealt once at spawn from the table's average, what it grows | `ANIMAL_LV_HP`, `animalLevel` (under `ANIMAL_HP`), `a.level` in `makeAnimal` | `animals` (the wolf's bite growth: `WOLF_LV_DMG`, above the `wolves` banner) |
