...
| the plates' own art: the merchant's gold sack and the three tails | `SPRITES.goldSack` (six 32x32 frames: `sackA`..`sackF`/`SACKPAL`, the `gold sack` section of sprites.js), `NOTE_TAILS` (up, down, crate), `NOTE_CRATE_PAL` | `market notices` (the tails stamped with `stampGrid`: `the art`, screens.js) |
